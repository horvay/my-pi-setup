...
```bash
node $HOME/.pi/agent/skills/impeccable/scripts/live.mjs
```
...
```
LOOP:
  node $HOME/.pi/agent/skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node $HOME/.pi/agent/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node $HOME/.pi/agent/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node $HOME/.pi/agent/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node $HOME/.pi/agent/skills/impeccable/scripts/live-server.mjs stop
```
...
```bash
node $HOME/.pi/agent/skills/impeccable/scripts/detect-csp.mjs
```
